...
Free-form attributes (include only if explicitly stated):
- color: exact color mentioned or clearly visible in image
- fabric: material explicitly stated or clearly visible
- visual_detail: any visual detail not expressible by the above (e.g. pattern, graphic, special trim)
...
class IntentAnalysisModule:
    ...
    def __init__(self, model_name=MODEL_NAME, tp=2, session_len=32768, cache_max_entry_count=0.8):
        """
        Args:
            model_name: VLM 모델 이름
            tp: Tensor parallelism 수
            session_len: 세션 길이
            cache_max_entry_count: 캐시 최대 엔트리 수
        """
    ...
    def infer_single(self, user_query: str, image_path: str = None, image_url: str = None) -> dict:
        ...
        try:
            if image:
                outputs = self.pipe([([image], final_prompt)], gen_config=self.gen_config)
            else:
                outputs = self.pipe([([], final_prompt)], gen_config=self.gen_config)
...
def main():
    output_path = os.path.join(os.path.dirname(__file__), "simple_test_results.json")
